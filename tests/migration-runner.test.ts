...
describe('Notion Migration Runner (Phase 1 Dry-Run & Planning)', () => {
  ...
  describe('12. Relation Wire Serialization, Plan Immutability & Safe Recovery Apply', () => {
    ...
    function createLiveSnapshotPostStepN(n: number): Record<string, Record<string, any>> {
      const plan = loadHomologatedPlan();
      const snapshot = createLiveSnapshotPostStep10();
      for (let s = 11; s <= n; s++) {
        const stepDef = plan.schemaPlan.steps.find((st: any) => st.stepNumber === s);
        if (!stepDef) continue;
        if (stepDef.operation === 'CREATE_PROPERTY' && stepDef.property) {
          const propKey = stepDef.property;
          const wire = materializeNotionApiPayload(stepDef.sanitizedPayload);
          const propConfig = wire[propKey];
          const type = Object.keys(propConfig)[0];
          snapshot[stepDef.targetDataSource.envKey][propKey] = {
            id: `prop_${propKey}_id`,
            name: propKey,
            type,
            ...propConfig,
          };
        } else if (stepDef.operation === 'CREATE_DUAL_RELATION') {
          snapshot.NOTION_DS_TRANSACTIONS['Fatura Vinculada'] = {
            id: 'prop_fatura_vinc_id',
            name: 'Fatura Vinculada',
            type: 'relation',
            relation: {
              data_source_id: 'mock-ds-id',
              dual_property: { synced_property_name: 'Lançamentos do Ciclo' },
            },
          };
        }
    ...
    function setupJournalWithTerminalSteps(journal: MigrationJournal, plan: any, upToStep: number) {
      journal.savePlan(plan, 'feat/phase-1-schema-apply-executor');
      for (let s = 1; s <= upToStep; s++) {
        ...
        journal.recordStepPending({
          ...
          targetDataSource: stepDef.targetDataSource.name,
          targetDataSourceId: stepDef.targetDataSource.id,
          propertyName: stepDef.property,
        });
        if (stepDef.operation === 'CREATE_DATABASE') {
          journal.recordStepVerified(HOMOLOGATED_RECOVERY_PLAN_HASH, s, 'mock-db-id');
        } else if (stepDef.operation === 'RESOLVE_DATA_SOURCE_ID') {
          journal.recordStepVerified(HOMOLOGATED_RECOVERY_PLAN_HASH, s, 'mock-ds-id');
        } else {
          journal.recordStepVerified(HOMOLOGATED_RECOVERY_PLAN_HASH, s, `prop_${stepDef.property || s}_id`);
        }
      }
    }
    ...
    it('Recovery Gate: drift live com propriedade posterior já presente no Notion bloqueia recuperação', async () => {
      ...
      // Live snapshot already has Step 12 (posterior to frontier Step 11) live!
      const driftedSnapshot = createLiveSnapshotPostStep10();
      driftedSnapshot.NOTION_DS_TRANSACTIONS['Status de Revisão'] = {
        id: 'prop_drift_12_id',
        name: 'Status de Revisão',
        type: 'select',
        select: { options: [{ name: 'Pendente Revisão' }] },
      };
      ...
      await expect(runner.execute(HOMOLOGATED_RECOVERY_PLAN_HASH)).rejects.toThrow('EXTERNAL_DRIFT_DETECTED');
      expect(fakeNotion.writeCallsCount).toBe(0);
    });
    ...
    // -------------------------------------------------------------------------
    // Sub-suite E: Stateful Interruption & Resume Recovery Hardening
    // -------------------------------------------------------------------------
    describe('Sub-suite E: Stateful Interruption & Resume Recovery', { timeout: 30000 }, () => {
      it('1. Crash após Step 11: preflight detecta RESUME_RECOVERY (11/54), Step 12 como frontier, e execute completa sem writes nos passos 1..11', async () => {
        const plan = loadHomologatedPlan();
        const journal = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journal, plan, 11);

        const postStep11Snapshot = createLiveSnapshotPostStepN(11);
        const fakeNotion = new StatefulNotionFake(postStep11Snapshot);
        ...
        const runner = new TestableMigrationRunner(
          ...
          },
          {
            ...
            gitCommitShaOverride: recoveryPatchSha,
            gitParentCommitShaOverride: HOMOLOGATED_RECOVERY_FROM_COMMIT,
            liveSnapshotOverride: postStep11Snapshot,
          },
        );
        runner.setClient(fakeNotion);

        const preflight = await runner.runRecoveryPreflight(HOMOLOGATED_RECOVERY_PLAN_HASH);
        expect(preflight.recoveryPhase).toBe('RESUME_RECOVERY');
        expect(preflight.journalStepsCompleted).toBe(11);
        expect(preflight.frontierStepNumber).toBe(12);
        expect(preflight.frontierStepProperty).toBe('Status de Revisão');
        expect(preflight.eligibility.eligible).toBe(true);
        expect(fakeNotion.writeCallsCount).toBe(0);

        // Execute recovery apply
        const executeRunner = new TestableMigrationRunner(
          {
            mode: 'recovery',
            dbPath: testDbPath,
            backupDir: testBackupDir,
        ...
        executeRunner.setClient(fakeNotion);

        const report = await executeRunner.execute(HOMOLOGATED_RECOVERY_PLAN_HASH);
        expect(report.mode).toBe('apply');
        if (report.mode === 'apply') {
          expect(report.summary.verifiedCount).toBe(54);
          expect(fakeNotion.writeCallsByProperty['Conta Destino']).toBeUndefined();
          expect(fakeNotion.writeCallsByProperty['Status de Revisão']).toBe(1);
        }
      });

      it('2. Crash após propriedade intermediária (Passo 25): identifica RESUME_RECOVERY (25/54), Step 26 como frontier, e resume sem writes em 1..25', async () => {
        const plan = loadHomologatedPlan();
        const journal = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journal, plan, 25);

        const postStep25Snapshot = createLiveSnapshotPostStepN(25);
        const fakeNotion = new StatefulNotionFake(postStep25Snapshot);
        ...
        const runner = new TestableMigrationRunner(
          ...
          },
          {
            ...
            gitCommitShaOverride: recoveryPatchSha,
            gitParentCommitShaOverride: HOMOLOGATED_RECOVERY_FROM_COMMIT,
            liveSnapshotOverride: postStep25Snapshot,
          },
        );
        runner.setClient(fakeNotion);
        ...
        const preflight = await runner.runRecoveryPreflight(HOMOLOGATED_RECOVERY_PLAN_HASH);
        expect(preflight.recoveryPhase).toBe('RESUME_RECOVERY');
        expect(preflight.journalStepsCompleted).toBe(25);
        expect(preflight.frontierStepNumber).toBe(26);
        expect(preflight.eligibility.eligible).toBe(true);
      ...
      it('3. Uncertain write no frontier (Passo 11): PATCH teve sucesso antes do crash, journal permaneceu PENDING, runner recupera com NO_OP_VERIFIED e continua N+1 sem novo PATCH', async () => {
        const plan = loadHomologatedPlan();
        const journal = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journal, plan, 10);
        journal.recordStepPending({
          planHash: HOMOLOGATED_RECOVERY_PLAN_HASH,
          stepNumber: 11,
          operation: 'CREATE_PROPERTY',
        ...
        const liveWithStep11 = createLiveSnapshotPostStepN(11);
        const fakeNotion = new StatefulNotionFake(liveWithStep11);
        ...
        const runner = new TestableMigrationRunner(
          ...
          },
          {
            ...
            gitCommitShaOverride: recoveryPatchSha,
            gitParentCommitShaOverride: HOMOLOGATED_RECOVERY_FROM_COMMIT,
            liveSnapshotOverride: liveWithStep11,
          },
        );
        runner.setClient(fakeNotion);

        const preflight = await runner.runRecoveryPreflight(HOMOLOGATED_RECOVERY_PLAN_HASH);
        expect(preflight.recoveryPhase).toBe('INITIAL_RECOVERY');
        expect(preflight.journalStepsCompleted).toBe(10);
        expect(preflight.frontierStepNumber).toBe(11);
        expect(preflight.frontierStepStatus).toBe('PENDING');
        expect(preflight.eligibility.eligible).toBe(true);
        expect(fakeNotion.writeCallsCount).toBe(0);
        ...
        const report = await executeRunner.execute(HOMOLOGATED_RECOVERY_PLAN_HASH);
        if (report.mode === 'apply') {
          const step11Status = journal.getStepStatus(HOMOLOGATED_RECOVERY_PLAN_HASH, 11);
          expect(step11Status?.status).toBe('NO_OP_VERIFIED');
          const meta = JSON.parse(step11Status?.metadataJson || '{}');
          expect(meta.recoveryReason).toBe('RECOVERED_AFTER_UNCERTAIN_WRITE');

          expect(fakeNotion.writeCallsByProperty['Conta Destino']).toBeUndefined();
          expect(fakeNotion.writeCallsByProperty['Status de Revisão']).toBe(1);
          expect(report.summary.verifiedCount).toBe(53);
          expect(report.summary.noOpCount).toBe(1);
        }
      });

      it('4. Crash após Step 52 (criação da database): reconcilia database existente, não recria base, e executa Steps 53 e 54', async () => {
        const plan = loadHomologatedPlan();
        const journal = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journal, plan, 52);

        const postStep51Snapshot = createLiveSnapshotPostStepN(51);
        const fakeNotion = new StatefulNotionFake(postStep51Snapshot);
        const createDbStep = plan.schemaPlan.steps.find((s: any) => s.operation === 'CREATE_DATABASE');
        const wirePayload = materializeNotionApiPayload(createDbStep.sanitizedPayload);
        await fakeNotion.databases.create({
          parent: { type: 'page_id', page_id: recoveryParentPageId },
          title: [{ type: 'text', text: { content: 'Faturas / Ciclos de Cartão' } }],
          description: [{ type: 'text', text: { content: `MIGRATION_MARKER:${HOMOLOGATED_RECOVERY_PLAN_HASH}:CARD_BILLS_V1` } }],
          initial_data_source: wirePayload.initial_data_source,
        });
        ...
        const runner = new TestableMigrationRunner(
          ...
          },
          {
            ...
            gitCommitShaOverride: recoveryPatchSha,
            gitParentCommitShaOverride: HOMOLOGATED_RECOVERY_FROM_COMMIT,
            liveSnapshotOverride: postStep51Snapshot,
          },
        );
        runner.setClient(fakeNotion);
        ...
        const preflight = await runner.runRecoveryPreflight(HOMOLOGATED_RECOVERY_PLAN_HASH);
        expect(preflight.recoveryPhase).toBe('RESUME_RECOVERY');
        expect(preflight.journalStepsCompleted).toBe(52);
        expect(preflight.frontierStepNumber).toBe(53);
        expect(preflight.eligibility.eligible).toBe(true);
        ...
        if (report.mode === 'apply') {
          expect(report.summary.totalSteps).toBe(54);
          expect(report.summary.verifiedCount + report.summary.noOpCount).toBe(54);
          expect(fakeNotion.writeCallsByProperty['Fatura Vinculada']).toBe(1);
        }
      });

      it('5. Crash após Step 53 (data_source_id resolvido): resume diretamente no Step 54 e completa migração com 54 passos', async () => {
        const plan = loadHomologatedPlan();
        const journal = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journal, plan, 53);
        ...
        const preflight = await runner.runRecoveryPreflight(HOMOLOGATED_RECOVERY_PLAN_HASH);
        expect(preflight.recoveryPhase).toBe('RESUME_RECOVERY');
        expect(preflight.journalStepsCompleted).toBe(53);
        expect(preflight.frontierStepNumber).toBe(54);
        expect(preflight.frontierStepProperty).toBe('Fatura Vinculada');
        expect(preflight.eligibility.eligible).toBe(true);
      ...
      it('6. Resume direto em Step 21 quando Steps 1..20 já constam terminais: Steps 1..20 recebem zero writes e Step 21 inicia execução', async () => {
        const plan = loadHomologatedPlan();
        const journal = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journal, plan, 20);

        const postStep20Snapshot = createLiveSnapshotPostStepN(20);
        const fakeNotion = new StatefulNotionFake(postStep20Snapshot);
        ...
        const runner = new TestableMigrationRunner(
          ...
          },
          {
            ...
            gitCommitShaOverride: recoveryPatchSha,
            gitParentCommitShaOverride: HOMOLOGATED_RECOVERY_FROM_COMMIT,
            liveSnapshotOverride: postStep20Snapshot,
          },
        );
        runner.setClient(fakeNotion);
        ...
        const preflight = await runner.runRecoveryPreflight(HOMOLOGATED_RECOVERY_PLAN_HASH);
        expect(preflight.recoveryPhase).toBe('RESUME_RECOVERY');
        expect(preflight.journalStepsCompleted).toBe(20);
        expect(preflight.frontierStepNumber).toBe(21);
        expect(preflight.frontierStepProperty).toBe('Data de Término');
        expect(preflight.eligibility.eligible).toBe(true);
        ...
        if (report.mode === 'apply') {
          expect(report.summary.verifiedCount).toBe(54);
          expect(fakeNotion.writeCallsByProperty['Conta Destino']).toBeUndefined();
          expect(fakeNotion.writeCallsByProperty['Status de Revisão']).toBeUndefined();
          expect(fakeNotion.writeCallsByProperty['Data de Término']).toBe(1);
        }
      });

      it('7. Rejeição de modificações posteriores não explicadas: journal com gap/posterior ou live com propriedade incompatível', async () => {
        const plan = loadHomologatedPlan();

        // 7a. Journal com passo posterior marcado como VERIFIED fora de ordem
        const journalA = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journalA, plan, 10);
        journalA.recordStepPending({
          planHash: HOMOLOGATED_RECOVERY_PLAN_HASH,
          stepNumber: 11,
          operation: 'CREATE_PROPERTY',
          targetDataSource: 'Transações',
        ...
        journalA.recordStepPending({
          ...
          stepNumber: 15,
          operation: 'CREATE_PROPERTY',
          targetDataSource: 'Regras de Classificação',
        });
        journalA.recordStepVerified(HOMOLOGATED_RECOVERY_PLAN_HASH, 15);

        const runnerA = new TestableMigrationRunner(
          {
            mode: 'recovery',
            dbPath: testDbPath,
            backupDir: testBackupDir,
        ...
        runnerA.setClient(new StatefulNotionFake(createLiveSnapshotPostStep10()));

        await expect(runnerA.execute(HOMOLOGATED_RECOVERY_PLAN_HASH)).rejects.toThrow('JOURNAL_INVALID');

        // 7b. Live workspace com propriedade do frontier em formato incompatível (ex: rich_text em vez de relation)
        const journalB = new MigrationJournal(testDbPath);
        setupJournalWithTerminalSteps(journalB, plan, 10);
        journalB.recordStepPending({
          planHash: HOMOLOGATED_RECOVERY_PLAN_HASH,
          stepNumber: 11,
          operation: 'CREATE_PROPERTY',
          targetDataSource: 'Transações',
        ...
        const incompatibleSnapshot = createLiveSnapshotPostStep10();
        incompatibleSnapshot.NOTION_DS_TRANSACTIONS['Conta Destino'] = {
          id: 'prop_incompat_id',
          name: 'Conta Destino',
          type: 'rich_text',
          rich_text: {},
        ...
        const runnerB = new TestableMigrationRunner(
          {
            mode: 'recovery',
            dbPath: testDbPath,
            backupDir: testBackupDir,
          ...
          },
          {
            ...
            gitCommitShaOverride: recoveryPatchSha,
            gitParentCommitShaOverride: HOMOLOGATED_RECOVERY_FROM_COMMIT,
            liveSnapshotOverride: incompatibleSnapshot,
          },
        );
        runnerB.setClient(new StatefulNotionFake(incompatibleSnapshot));

        await expect(runnerB.execute(HOMOLOGATED_RECOVERY_PLAN_HASH)).rejects.toThrow('EXTERNAL_DRIFT_DETECTED');
      });
    });
  });
});
