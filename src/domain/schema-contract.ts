...
export const TARGET_CONTRACT: Record<string, DataSourceContract> = {
  ...
  NOTION_DS_CATEGORIES: {
    ...
    properties: [
      ...
      { domainField: 'budgetGroup', notionProperty: 'Grupo Orçamentário', notionType: 'select', direction: 'both', authority: 'USUARIO', description: 'Agrupamento macro orçamentário configurável (ex: Essencial, Estilo de Vida, Metas, Estrutural)' },
  ...
  NOTION_DS_INVESTMENTS: {
    ...
    properties: [
      ...
      { domainField: 'unitAveragePrice', notionProperty: 'Preço Médio Unitário (PMP)', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Preço médio ponderado unitário de aquisição' },
      { domainField: 'totalCostBasis', notionProperty: 'Custo Base Total', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Montante total acumulado desembolsado nas compras (base de custo contábil)' },
      { domainField: 'currentMarketValue', notionProperty: 'Valor de Mercado Atual', notionType: 'number', direction: 'both', authority: 'PIERRE', description: 'Posição a mercado: Cotação atual x quantidade' },
      { domainField: 'unrealizedProfitLoss', notionProperty: 'Lucro / Prejuízo Não Realizado', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Valor de Mercado Atual - Custo Base Total' },
      { domainField: 'profitLossPercentage', notionProperty: 'Retorno Não Realizado (%)', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: '((Valor de Mercado Atual - Custo Base Total) / Custo Base Total) * 100' },
  ...
  NOTION_DS_MONTHLY_BUDGET: {
    ...
    properties: [
      ...
      { domainField: 'budgetGroupTargets', notionProperty: 'Metas por Grupo Orçamentário', notionType: 'rich_text', direction: 'both', authority: 'USUARIO', description: 'Metas percentuais ou tetos nominais por grupo orçamentário configurável' },
      { domainField: 'savingsRateTarget', notionProperty: 'Meta Taxa de Poupança (%)', notionType: 'number', direction: 'both', authority: 'USUARIO', description: 'Percentual alvo da renda líquida destinado a investimentos e reservas' },
  ...
  NOTION_DS_MONTHLY_CLOSINGS: {
    ...
    properties: [
      ...
      { domainField: 'totalExpenses', notionProperty: 'Despesas Consolidadas', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Total de despesas correntes de subsistência e consumo' },
      { domainField: 'essentialExpenses', notionProperty: 'Despesas Essenciais (Necessidades)', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Gastos essenciais de moradia, saúde, alimentação básica' },
      { domainField: 'discretionaryExpenses', notionProperty: 'Despesas Discricionárias (Desejos)', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Gastos de estilo de vida, lazer e compras' },
      { domainField: 'operatingSurplus', notionProperty: 'Resultado do Mês (Sobra Operacional)', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Renda Consolidada - Despesas Consolidadas (fluxo de caixa operacional livre do mês antes de aportes)' },
      { domainField: 'savingsAndInvestments', notionProperty: 'Poupança / Aportes Realizados', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Total efetivamente poupado e transferido para investimentos no mês' },
      { domainField: 'savingsRate', notionProperty: 'Taxa de Poupança (%)', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: '(Poupança / Aportes Realizados / Renda Consolidada) * 100' },
      { domainField: 'unallocatedCashFlow', notionProperty: 'Fluxo Residual Não Alocado', notionType: 'number', direction: 'write', authority: 'DERIVADO', description: 'Sobra Operacional - Poupança/Aportes Realizados' },
  ...
  NOTION_DS_SYNC_LOG: {
    ...
    properties: [
      ...
      { domainField: 'runId', notionProperty: 'ID da Execução (Run ID)', notionType: 'rich_text', direction: 'write', authority: 'DERIVADO', description: 'UUID unívoco da execução do worker' },
      { domainField: 'errorCode', notionProperty: 'Código do Erro', notionType: 'select', direction: 'write', authority: 'DERIVADO', description: 'Código padronizado de erro (ex: AUTH_EXPIRED, NETWORK_TIMEOUT, VALIDATION_ERROR)' },
      { domainField: 'sanitizedErrorMessage', notionProperty: 'Mensagem de Erro Sanitizada', notionType: 'rich_text', direction: 'write', authority: 'DERIVADO', description: 'Mensagem descritiva tratada, sem stack trace ou dados sensíveis' },
      { domainField: 'privateLogRef', notionProperty: 'Referência do Log Privado', notionType: 'rich_text', direction: 'write', authority: 'DERIVADO', description: 'Identificador do registro detalhado no armazenamento privado cifrado' },
