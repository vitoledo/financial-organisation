...
async function main() {
  ...
  const resolvedCount = report.transactionAudits.length - unresolvedTx.length;

  console.log('───────────────────────────────────────────────────────────────────────────────');
  console.log(`  1. RESOLUÇÃO DETERMINÍSTICA DAS CONTAS DE ORIGEM (${resolvedCount}/${report.transactionAudits.length} RESOLVIDAS)`);
  console.log('───────────────────────────────────────────────────────────────────────────────');
  console.log(`  • Transações de Conta Corrente (Nubank Conta): ${nubankContaTx.length}`);
  console.log(`    - Método de Resolução: SOURCE_ACCOUNT_ID (Mapeado via BACKFILL_ACCOUNT_MAPPING_PATH)`);
  ...
  console.log(`\n  [A] Fluxo de Caixa Físico — Conta Corrente (Nubank Conta, ${nubankContaTx.length} txs):`);
  console.log(`      • Entradas Físicas (+): R$ ${r.checkingCashFlow.inflowsTotal.toFixed(2)}`);
  console.log(`          - Entradas de Terceiros (${r.inflowsAuditSummary.thirdPartyInflowsCount} txs pendentes): R$ ${r.checkingCashFlow.thirdPartyInflows.toFixed(2)}`);
  console.log(`          - Transferências Mesma Titularidade (${r.inflowsAuditSummary.sameOwnershipInflowsCount} tx entrada): R$ ${r.checkingCashFlow.sameOwnershipInflows.toFixed(2)}`);
  console.log(`      • Saídas Físicas (-): R$ ${r.checkingCashFlow.totalOutflows.toFixed(2)}`);
  console.log(`          - Despesas Diretas de Conta (Confirmadas): R$ ${r.checkingCashFlow.directOutflows.toFixed(2)}`);
  console.log(`          - Transferências Mesma Titularidade (Saídas): R$ ${r.checkingCashFlow.outgoingInternalTransfers.toFixed(2)}`);
  console.log(`          - Transferências Enviadas a Terceiros (Pendentes de Revisão): R$ ${(r.checkingCashFlow.pendingOutflows || 0).toFixed(2)}`);
  ...
  console.log(`\n  [B] Variação de Passivo de Cartão de Crédito (Nubank Cartão, ${nubankCartaoTx.length} txs):`);
  ...
  console.log(`      • Despesas Econômicas Diretas (Conta Corrente, deduzindo R$ ${r.checkingCashFlow.outgoingInternalTransfers.toFixed(2)} internas e R$ ${(r.checkingCashFlow.pendingOutflows || 0).toFixed(2)} pendentes): R$ ${r.economicConsumption.directCheckingExpenses.toFixed(2)}`);
  console.log(`      • Despesas Econômicas de Cartão (Compras): R$ ${r.economicConsumption.cardPurchases.toFixed(2)}`);
  console.log(`      • Despesas Econômicas Confirmadas (1): R$ ${(r.economicConsumption.confirmedEconomicExpenses || r.economicConsumption.totalEconomicExpenses).toFixed(2)}`);
  console.log(`      • Saídas Econômicas Pendentes de Classificação (2): R$ ${(r.economicConsumption.pendingEconomicOutflows || 0).toFixed(2)} (${report.planArtifact.readiness.pendingCategoryReviewCount} transferências enviadas)`);
  console.log(`      • Desembolso de Caixa Físico Total (3): R$ ${(r.economicConsumption.physicalCashOutflows || r.checkingCashFlow.totalOutflows).toFixed(2)}`);
  console.log(`      • Receitas Econômicas Confirmadas: R$ ${r.economicConsumption.economicIncome.toFixed(2)}`);
  console.log(`      • Entradas de Terceiros Pendentes de Classificação: R$ ${r.economicConsumption.pendingThirdPartyInflows.toFixed(2)} (${r.inflowsAuditSummary.thirdPartyInflowsCount} txs — Status: Pendente Revisão)`);
  ...
  // 3. AUDITORIA DAS OCORRÊNCIAS DE PAGAMENTO DE CARTÃO
  console.log('───────────────────────────────────────────────────────────────────────────────');
  console.log(`  3. AUDITORIA DAS ${r.paymentAuditSummary.totalPaymentOccurrences} OCORRÊNCIAS DE PAGAMENTO DE CARTÃO E RECONCILIAÇÃO`);
  console.log('───────────────────────────────────────────────────────────────────────────────');
  ...
  console.log(`  • Total de Eventos de Pagamento Reconciliados: ${report.paymentEventAllocations.length}`);
  console.log('  • Vínculos Canônicos com Faturas:');
  console.log(`      - ${report.paymentEventAllocations.length} eventos de pagamento vinculam-se a Faturas.Transações de Pagamento`);
  console.log(`      - 0 pagamentos de cartão vinculam-se a Transações.Fatura Vinculada (exclusivo para ${r.cardLiability.purchasesCount} compras)\n`);
  ...
  console.log(`  [B] Auditoria Individual das ${report.paymentLegAudits.length} Ocorrências de Pagamento:`);
  ...
  // 4. AUDITORIA DAS ENTRADAS E TRANSFERÊNCIAS RECEBIDAS NO BANCO
  const inAudit = r.inflowsAuditSummary;
  console.log('───────────────────────────────────────────────────────────────────────────────');
  console.log(`  4. AUDITORIA DAS ${inAudit.totalInflows} ENTRADAS E TRANSFERÊNCIAS RECEBIDAS NO BANCO`);
  console.log('───────────────────────────────────────────────────────────────────────────────');
  console.log(`  • Total de Entradas no Banco Analisadas: ${inAudit.totalInflows}`);
  console.log(`  • Transferências Mesma Titularidade: ${inAudit.sameOwnershipInflowsCount} (R$ ${r.checkingCashFlow.sameOwnershipInflows.toFixed(2)} — Neutro / Confirmado Auto)`);
  ...
  console.log(`      - ${inAudit.thirdPartyInflowsCount} entradas de terceiros têm Natureza = null, Efeito Orçamentário = null,`);
  ...
  console.log('| :---: | :---: | :---: | :---: | :--- | :--- | :--- | :--- |');
  const hmacKey = process.env.COUNTERPARTY_HMAC_KEY;
  const hmacVersion = process.env.COUNTERPARTY_HMAC_KEY_VERSION || 'v1';
  ...
  console.log('───────────────────────────────────────────────────────────────────────────────');
  console.log(`  5. AUDITORIA DOS ${report.cardBillAudits.length} CICLOS DE FATURA DE CARTÃO E PROVENIÊNCIA DE CAMPOS`);
  console.log('───────────────────────────────────────────────────────────────────────────────');
  ...
  for (const b of report.cardBillAudits) {
    ...
    const statusStr = (b.status || 'null (sem fatura)').padEnd(18, ' ');
    console.log(
      `| ${b.stableBillId.padEnd(42, ' ')} | ${b.inicio} | ${b.fim} | ${b.fechamento} | ${b.vencimento} | ${statusStr} | ${b.qualidade.padEnd(18, ' ')} | ${b.nCompras.toString().padStart(2, ' ')} | R$ ${b.somaCompras.toFixed(2).padStart(6, ' ')} | R$ ${b.paidAmount.toFixed(2).padStart(6, ' ')} | R$ ${b.purchasePaymentDelta.toFixed(2).padStart(6, ' ')} | ${discOficialStr.padEnd(16, ' ')} |`,
    );
  }
  ...
  console.log('| Status da Fatura | CONSERVADOR | null (ausência de fatura oficial no upstream; conservador) |');
  console.log('| Início / Fim / Fechamento / Vencimento | SOURCE/DERIVED/CONFIG | Datas upstream ou derivadas de metadados da conta / config |');
  console.log(`| Total de Compras no Ciclo | DERIVED | Soma estrita das compras do ciclo (R$ ${r.cardLiability.totalPurchases.toFixed(2)} total) |`);
  console.log(`| Lançamentos do Ciclo | DERIVED | Relação com exatamente ${r.cardLiability.purchasesCount} compras (STAGE_2) |`);
  console.log(`| Transações de Pagamento | DERIVED | Relação com ${report.paymentEventAllocations.length} eventos de pagamento reconciliados (STAGE_2) |\n`);
  ...
  console.log(`  • Transações com ID Estável da Fonte (Source ID): ${idS.countWithSourceId}/${report.totalSourceTransactions} (100%)`);
  ...
  console.log(`      - ${report.totalSourceTransactions} Transações (sem links entre novas entidades)`);
