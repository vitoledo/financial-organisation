...
> **Data da Verificação:** 2026-09-13T18:45:51.253Z
> **Notion API Version:** `2026-03-11`
> **Data Sources Canônicos:** 13 (13 esperados existentes + 1 base proposta)
...
| Bases Existentes Esperadas | 13 |
| Bases Configuradas no Ambiente | 13 |
| Bases Verificadas com Sucesso na API | 13 |
| Bases com Falha / Não Verificadas | 0 |
| Base Proposta (a criar externamente) | 1 |

| Status das Propriedades | Quantidade |
| :--- | :--- |
| Correspondência Exata (EXACT_MATCH) | 142 |
...
| Propriedades a Criar na 13ª Base (PROPOSED_TO_CREATE) | 0 |
...
| `Conta Destino` | `relation` | `relation` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Conta de destino para transferências internas entre contas |
| `Categoria` | `relation` | `relation` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Relação com Categorias Financeiras |
| `Fatura Vinculada` | `relation` | `relation` | ✅ EXACT_MATCH | `DERIVADO` | Relação com 13ª base Faturas / Ciclos |
| `Status de Revisão` | `select` | `select` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Confirmado Auto, Pendente Revisão, Validado Manualmente, Legado Não Verificado |
| `Motivo da Revisão` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Justificativa para intervenção humana |
...
| `HMAC Contraparte` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `DERIVADO` | Hash seguro do CPF/CNPJ para matching sem expor PII |
...
| `Atribuir: Efeito Orçamento` | `select` | `select` | ✅ EXACT_MATCH | `USUARIO` | INCOME, EXPENSE, REVERSAL, NEUTRAL |
| `Atribuir: Alocação` | `select` | `select` | ✅ EXACT_MATCH | `USUARIO` | Finalidade da movimentação |
| `Atribuir: Conta Destino` | `relation` | `relation` | ✅ EXACT_MATCH | `USUARIO` | Conta de destino atribuída para transferências internas |
...
| `Competência Âncora` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `USUARIO` | Mês de referência inicial (ex: 2026-01) |
| `Última Competência Gerada` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Último mês gerado em Obrigações (ex: 2026-08) |
...
| `Data de Início` | `date` | `date` | ✅ EXACT_MATCH | `USUARIO` | Início da vigência do contrato |
| `Data de Término` | `date` | `date` | ✅ EXACT_MATCH | `USUARIO` | Término da vigência do contrato |
...
| `Custódia` | `select` | `select` | ✅ EXACT_MATCH | `USUARIO` | Conta Pierre ou Carteira Externa |
...
| `Preço Médio Unitário (PMP)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Preço médio ponderado unitário de aquisição |
| `Lucro / Prejuízo Não Realizado` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Valor de Mercado Atual - Custo Base Total |
| `Retorno Não Realizado (%)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | ((Valor de Mercado Atual - Custo Base Total) / Custo Base Total) * 100 |
...
| `Conta Vinculada` | `relation` | `relation` | ✅ EXACT_MATCH | `USUARIO` | Conta corrente ou corretora associada |
...
| `Impacto Líquido de Caixa` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Efeito líquido de caixa com sinal econômico direcional (compra = negativo, venda = positivo) |
...
| `Conta Destino / Caixa` | `relation` | `relation` | ✅ EXACT_MATCH | `DERIVADO` | Conta creditada |
...
| Propriedade | Tipo Esperado | Tipo no Notion | Status | Autoridade | Descrição / Observação |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `Receitas Realizadas` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Soma de transações OPERATING_REVENUE |
| `Despesas Realizadas` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Soma de transações OPERATING_EXPENSE |
| `Poupança Realizada` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Soma de transações com savingsGoalContribution |
| `Compras Realizadas Cartão` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Soma de compras de cartão no mês |
...
| Propriedade | Tipo Esperado | Tipo no Notion | Status | Autoridade | Descrição / Observação |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `Status de Reconciliação` | `select` | `select` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Conciliado Integralmente, Divergências Pendentes, Reconciliação Manual Necessária |
...
| `Patrimônio Inicial` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Patrimônio líquido consolidado no início do mês |
| `Despesas Essenciais (Necessidades)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Gastos essenciais de moradia, saúde, alimentação básica |
| `Despesas Discricionárias (Desejos)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Gastos de estilo de vida, lazer e compras |
| `Resultado do Mês (Sobra Operacional)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Renda Consolidada - Despesas Consolidadas (fluxo de caixa operacional livre do mês antes de aportes) |
| `Taxa de Poupança (%)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | (Poupança / Aportes Realizados / Renda Consolidada) * 100 |
| `Fluxo Residual Não Alocado` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Sobra Operacional - Poupança/Aportes Realizados |
| `Rendimentos de Investimentos` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Rendimentos e dividendos auferidos no mês |
| `Variação Patrimonial` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Patrimônio Final - Patrimônio Inicial |
...
| `Duração (s)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Tempo total de processamento em segundos |
| `Duração (ms)` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Tempo total de processamento em milissegundos |
...
| `Transações Inalteradas` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Transações sem alteração ignoradas |
| `Erros Encontrados` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Total de falhas ou exceções não fatais registradas durante o lote |
| `Parcelas recebidas` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Parcelamentos e faturas futuras identificadas |
| `Obrigações Conciliadas` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Compromissos mensais liquidados com sucesso |
| `Freshness da fonte` | `date` | `date` | ✅ EXACT_MATCH | `DERIVADO` | Momento do último dado retornado pelo banco / open finance |
| `ID da Execução (Run ID)` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `DERIVADO` | UUID unívoco da execução do worker |
| `Código do Erro` | `select` | `select` | ✅ EXACT_MATCH | `DERIVADO` | Código padronizado de erro (ex: AUTH_EXPIRED, NETWORK_TIMEOUT, VALIDATION_ERROR, CONCURRENCY_LOCKED) |
| `Referência do Log Privado` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `DERIVADO` | Identificador do registro detalhado no armazenamento privado cifrado |
| `Hash do Lote RAW` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `DERIVADO` | SHA-256 do payload bruto salvo no cofre cifrado |
| `Versão do Worker / Commit` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `DERIVADO` | Commit SHA ou tag da versão do worker em execução |
...
* **Status:** `CONFIGURED_AND_VERIFIED`
* **Data Source ID (Parcial):** `edc7...1b0b`

| Propriedade | Tipo Esperado | Tipo no Notion | Status | Autoridade | Descrição / Observação |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `Fatura / Ciclo` | `title` | `title` | ✅ EXACT_MATCH | `DERIVADO` | Ex: Nubank - Ciclo 2026-07 (Venc 22/07) |
| `Fonte` | `select` | `select` | ✅ EXACT_MATCH | `UPSTREAM` | Conector de proveniência da fatura (PIERRE, MANUAL, MIGRATION, OTHER) - instituição bancária vem de Cartão Vinculado |
| `ID da Fatura na Fonte` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `UPSTREAM` | Identificador primário bruto retornado pela API ou fatura externa |
| `ID Estável da Fatura` | `rich_text` | `rich_text` | ✅ EXACT_MATCH | `DERIVADO` | Chave canônica unívoca (source:account_id:bill_id ou fallback completo) |
| `Qualidade da Identidade` | `select` | `select` | ✅ EXACT_MATCH | `DERIVADO` | SOURCE_ID ou PERIOD_FALLBACK para reconciliação/rekeying |
| `Cartão Vinculado` | `relation` | `relation` | ✅ EXACT_MATCH | `DERIVADO` | Relação com a conta do cartão em Contas |
| `Moeda` | `select` | `select` | ✅ EXACT_MATCH | `UPSTREAM` | Código da moeda da fatura (BRL, USD, EUR) |
| `Início do Período` | `date` | `date` | ✅ EXACT_MATCH | `DERIVADO` | Data inicial das compras elegíveis ao ciclo |
| `Fim do Período` | `date` | `date` | ✅ EXACT_MATCH | `DERIVADO` | Data final de corte das compras elegíveis ao ciclo |
| `Data de Fechamento` | `date` | `date` | ✅ EXACT_MATCH | `UPSTREAM` | Data oficial de corte da fatura emitida pelo banco |
| `Data de Vencimento` | `date` | `date` | ✅ EXACT_MATCH | `UPSTREAM` | Data oficial de vencimento da fatura emitida pelo banco |
| `Tipo de Ciclo` | `select` | `select` | ✅ EXACT_MATCH | `DERIVADO` | Ciclo Real Banco, Ciclo Configurado, Ciclo Estimado |
| `Origem / Qualidade dos Dados` | `select` | `select` | ✅ EXACT_MATCH | `DERIVADO` | Qualidade do valor da fatura: UPSTREAM_OFFICIAL, UPSTREAM_APPROXIMATE, DERIVED, MANUAL |
| `Status da Fatura` | `select` | `select` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Aberta em Curso, Fechada a Vencer, Vencida, Paga Integralmente, Paga Parcialmente |
| `Valor da Fatura Fechada (Oficial)` | `number` | `number` | ✅ EXACT_MATCH | `UPSTREAM` | Valor consolidado oficial emitido pelo banco após fechamento. Estritamente null enquanto fatura aberta. |
| `Valor Estimado da Fatura Aberta` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Projeção acumulada das compras correntes antes do corte |
| `Total de Compras no Ciclo` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Soma real das transações de compra dentro do ciclo corrente |
| `Componentes Adicionais da Fatura` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Soma dos componentes adicionais identificados (parcelas anteriores não contabilizadas no ciclo, encargos, IOF, juros, créditos/estornos) |
| `Divergência Não Explicada` | `number` | `number` | ✅ EXACT_MATCH | `DERIVADO` | Discrepância residual caso os componentes adicionais conhecidos não expliquem a diferença |
| `Valor Pago` | `number` | `number` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Valor total liquidado até o momento |
| `Data de Liquidação` | `date` | `date` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Data exclusiva da quitação integral da fatura |
| `Transações de Pagamento` | `relation` | `relation` | ✅ EXACT_MATCH | `REGRA_AUTOMATICA` | Relações com as transações bancárias de saída que quitaram ou amortizaram a fatura (suporta múltiplos pagamentos) |
| `Lançamentos do Ciclo` | `relation` | `relation` | ✅ EXACT_MATCH | `DERIVADO` | Relação dual bidirecional única sincronizada com Transações.Fatura Vinculada |

---

## 3. Status Canônico da 13ª Base: `Faturas / Ciclos de Cartão`

Esta base foi provisionada com sucesso pela migração DDL e encontra-se canonicamente configurada (`NOTION_DS_CARD_BILLS`).

---

## 4. Status de Conformidade do Workspace

Todas as 13 bases de dados canônicas estão configuradas e verificadas no Notion.
O schema DDL encontra-se 100% aderente ao contrato canônico (MISSING=0, STRUCTURAL_MISMATCH=0).
